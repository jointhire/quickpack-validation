...
class KatakanaCheckValidator < ActiveModel::EachValidator

  include Quickpack::Validation

  def validate_each(record, attribute, value)

    allow_word = CHAR_RANGE["zenkaku_katakana"]
    ...
    unless flg
      if options[:message].nil?
        record.errors.add(column_name, I18n.t('errors.messages.katakana_check'))
      else
